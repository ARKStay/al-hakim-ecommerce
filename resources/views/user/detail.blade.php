<x-layouts.layout>
            ...
            <div class="grid md:grid-cols-2 gap-10">
                ...
                <div>
                    ...
                    <div class="mt-6 w-full">
                        <form action="{{ url('user/detail/' . $product->slug) }}" method="POST" class="space-y-4"
                            id="addToCartForm">
                            @csrf

                            {{-- Quantity input baris sejajar --}}
                            ...
                            <button type="submit" id="addToCartBtn"
                                class="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded font-medium text-center block">
                                Add to Cart
                            </button>
                        </form>
                    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if (!form) return; // Kalau halaman ini nggak punya Add to Cart form, stop

            const addToCartBtn = document.getElementById("addToCartBtn");

            form.addEventListener("submit", function(e) {
                // Pastikan tombol yang diklik adalah Add to Cart
                if (e.submitter === addToCartBtn) {
                    ...
                    if (!selectedColor || !selectedSize) {
                        ...
                        Swal.fire({
                            ...
                            text: "Kindly select a color and size before adding this item to your cart.",
    ...
    <script>
        ...
        function updateTotalPrice() {
            ...
            if (qty > maxQty) {
                qty = maxQty;
                input.value = maxQty; // Kunci balik ke max
            } else if (qty < 1) {
                qty = 1;
                input.value = 1; // Jangan bisa isi negatif atau 0
            }
        ...
        function selectSize(size) {
            ...
            if (selected) {
                ...
                // ✅ Update max input quantity sesuai stock
                document.getElementById('quantity').max = selected.stock;

                // ✅ Update variant_id ke hidden input
                document.getElementById('selectedVariantInput').value = selected.id;
            }
