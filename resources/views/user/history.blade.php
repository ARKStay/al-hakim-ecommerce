<x-layouts.layout>
    ...
    <div class="container mx-auto mt-6">
        ...
        @forelse ($orders->where('order_status', 'complete') as $order)
            <div class="border border-gray-300 rounded-lg shadow-sm p-4 mb-6">
                ...
                <div class="flex justify-between items-center">
                    <div>
                        <span class="text-sm text-gray-500">Order Date: {{ $order->created_at->format('d M Y, H:i') }}</span>
                    </div>
                </div>
                    ...
                    @foreach ($order->cart->items as $item)
                        <div class="flex items-start gap-4 border-b border-gray-200 pb-4 mb-4">
                            ...
                            <div class="text-right">
                                <p class="text-sm font-medium text-gray-700">Rp {{ number_format($item->product->price, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="flex justify-between items-center mt-4">
                    ...
                    <span class="text-lg font-bold text-red-500">Rp {{ number_format($order->cart->items->sum(fn($item) => $item->product->price * $item->quantity), 0, ',', '.') }}</span>
                </div>

                <!-- Rate Order Button (hides if already rated) -->
                @if ($order->ratings->where('user_id', auth()->id())->isEmpty())
                    <div class="mt-4 text-right">
                        <button class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition" data-order-id="{{ $order->id }}" onclick="openRatingModal(this)">
                            Rate Order
                        </button>
                    </div>
                @else
        ...
        <p class="my-8 text-center text-sm text-gray-500">
            Please note: Your order history may be deleted after a few days.
        </p>
    ...
    <div id="ratingModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 hidden z-50">
        <div class="modal-content bg-white p-6 rounded-lg shadow-lg w-96 mx-auto mt-16 transform translate-y-1/2">
            ...
            <form id="ratingForm" action="{{ route('user.rating.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select id="rating" name="rating" class="mt-2 block w-full border border-gray-300 rounded-md shadow-sm">
                ...
                <div class="flex justify-end gap-2">
                    <button type="button" class="bg-gray-300 text-gray-700 px-4 py-2 rounded-md" onclick="closeRatingModal()">Cancel</button>
